...
/* ------------------------------------------------------------------
* Method: setExtendedColorZones(params)
* - params:
*   - colors_count | Integer | Required | 0 - 255
*   - zone_index   | Integer | Required | 0 - 255
*   - colors       | Object[]| Required | `Color` list
*   - duration     | Integer | Optional | The default value is 0 msec
*   - apply        | Integer | Optional | The default value is 1.
*                                         0: NO_APPLY, 1: APPLY, 2: APPLY_ONLY
*
...
*     - kelvin     | Integer | Optional | 1500 - 9000
* ---------------------------------------------------------------- */
LifxLanDevice.prototype.setExtendedColorZones = function (params) {
	let promise = new Promise((resolve, reject) => {
		if(!params) {
			throw new Error('The `params` is required.');
		} else if(typeof(params) !== 'object') {
		...
		let cList = params['colors'];
		if (!cList) {
			throw new Error('The `colors` are required.');
		} else if (typeof (cList) !== 'object') {
			throw new Error('The `colors` are invalid.');
		}
		this.multiZoneGetColorZones({
			start: params['zone_index'],
			end: params['colors_count']
		}).then((res) => {
