...
LifxLanComposer.prototype._composePayload = function(type, payload) {
	...
	} else if(type === 510) { // SetExtendedColorZones
...
/* ------------------------------------------------------------------
* Method: _composePayload510(payload) : SetExtendedColorZones
* - payload:
*   - colors_count  | Integer       | Required | Number of colors in the colors field to apply to the strip
*   - duration      | Integer       | Optional | Duration in milliseconds (default: 0 - infinite)
*   - zone_index	| Integer       | Required | The first zone to apply colors from.
...
LifxLanComposer.prototype._composePayload510 = function(payload) {
	...
	if('colors_count' in payload) {
		...
		if(typeof(colors_count) !== 'number' || colors_count % 1 !== 0 || colors_count < 0 || colors_count > 255) {
			return {error: new Error('The value of the `colors_count` must be an integer between 0 and 255.')};
		}
	} else {
	...
	// Check the `colors`
	let colors = [];
	if(!('colors' in payload) || typeof(payload['colors']) !== 'object') {
		return {error: new Error('The `colors` are required.')};
	}

	for (var i = 0; i < payload['colors'].length; i++) {
		let color_check_res = this._checkColorValues(payload['colors'][i]);
		if(color_check_res['error']) {
			return {error: color_check_res['error']};
		}
		colors.push(color_check_res['color']);
	}
	...
	// Compose a payload
	let buf = Buffer.alloc(8 + 8 * colors.length);
	buf.writeUInt32LE(duration, 0);
	buf.writeUInt8(apply, 4);
	buf.writeUInt16LE(zone_index, 5);
	buf.writeUInt8(colors_count, 7);
	for (var i = 0; i < colors.length; i++) {
		buf.writeUInt16LE(colors[i]['hue'], 8 + i * 8);
		buf.writeUInt16LE(colors[i]['saturation'], 10 + i * 8);
		buf.writeUInt16LE(colors[i]['brightness'], 12 + i * 8);
		buf.writeUInt16LE(colors[i]['kelvin'], 14 + i * 8);
	}
